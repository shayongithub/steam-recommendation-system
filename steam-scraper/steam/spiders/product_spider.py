...
def load_product(response):
    ...
    try:
        ...
        for line in details:
            ...
            for prop, name in [
                ...
                if prop in line:
                    if prop == "Developer:":
                        ind1 = line.find('Publisher:')
                        subline = line[:ind1]
                        item = subline.replace(prop, '').strip()
                        loader.add_value(name, item)
                    elif prop == "Publisher:":
                        ind1 = line.find('Publisher:')
                        ind2 = line.find('Release Date:')
                        subline = line[ind1:ind2]
                        item = subline.replace(prop, '').strip()
                        loader.add_value(name, item)
                    elif prop == "Release Date:":
                        ind1 = line.find('Release Date:')
                        subline = line[ind1:]
                        item = subline.replace(prop, '').strip()
                        loader.add_value(name, item)
                    else:
    ...
    loader.add_css('specs', '.game_area_details_specs_ctn ::text')
    ...
    loader.add_css('n_reviews', 'span.responsive_hidden ::text', re='\(([\d,]+)\)')
